...
#        #========================
    ...
    def extractC_wt(self,spectr,c,j):
        ...
        for k in range(self.nx):
            ...
            extr[k]=sum(Gauss(self.ff,f,dw)*spectr[k,:]*(self.T_len/self.L_len[j]*abs(self.kk[k])),0)/max(sum(Gauss(self.ff,f,dw)),1.)
...
#        return extr
    ...
    def get_diffu(self,mask_func,option='lonRes'):
        ...
        if option=='zm':
            results=[pool.apply_async(parse_uv_diffu,args=(kk,ff,um,va,mask_func,x,y,T_len,L_len)) for y in range(self.ny) for x in range(xlev,xlev+1)]
            output=[p.get() for p in results]
            output=np.array(output)
            diffu_spectr=output.reshape((self.ny,1,self.nx))
        elif option=='lonRes':
            results=[pool.apply_async(parse_uv_diffu,args=(kk,ff,um,va,mask_func,x,y,T_len,L_len)) for y in range(self.ny) for x in range(self.nx)]
            output=[p.get() for p in results]
            output=np.array(output)
            diffu_spectr=output.reshape((self.ny,self.nx,self.nx))
        ...
        #for y in range(self.ny):
            ...
            #for x in range(self.nx):
                #print 'x=',x,'y=',y
                #diffu_spectr[y,x,:]=self.parse_uv_diffu(mask_func,x,y)
...
#    def parse_uv_diffu(self,mask_func,x,y):
#        vMask=np.zeros([self.nx,self.nt])
#        u_mn=self.localU(x,y,mask_func)
#        for t in range(self.nt):
#            #=== apply mask ===
#            for xx in range(self.nx):
#                vMask[x+xx-self.nx,t]=self.va[t,y,x+xx-self.nx]*mask_func[xx]
#        vSpectr2=fft2(vMask)/(self.nx*self.nt)
#        #diffu_spectr[y,x,:]=self.extractC_wt2(abs(vSpectr2)**2,u_mn,y)
#        return self.extractC_wt2(abs(vSpectr2)**2,u_mn,y)

    #used nx,nt,localU and extractC_wt2
...
def uk2f(T_len,L_len,u,k):
    f=k*T_len/L_len*u
    return f

def localU(um,x,y,mask_func):
    nx=um.shape[1]
    uMask=np.zeros([nx])
    #==== obtain local U ====
    for xx in range(nx):
        uMask[x+xx-nx]=um[y,x+xx-nx]*mask_func[xx]
    return np.sum(uMask,0)/np.sum(mask_func)
    #========================

def extractC_wt2(kk,ff,spectr,c,T_len,L_len):
    nx=kk.shape[0]
    extr=np.zeros(nx)
    for k in range(nx):
        try:
            f=uk2f(T_len,L_len,c,kk[k])
        except:
            print kk
        extr[k]=np.sum(Gauss(ff,f,dw)*spectr[k,:]*(T_len/(2*np.pi)),0)/max(sum(Gauss(ff,f,dw)),1.)
    return extr

def parse_uv_diffu(kk,ff,um,va,mask_func,x,y,T_len,L_len):
    print 'x=',x,'y=',y
    nx=kk.shape[0]
    nt=ff.shape[0]
    vMask=np.zeros([nx,nt])
    u_mn=localU(um,x,y,mask_func)
    for t in range(nt):
        #=== apply mask ===
        for xx in range(nx):
            vMask[x+xx-nx,t]=va[t,y,x+xx-nx]*mask_func[xx]
    vSpectr2=fft2(vMask)/(nx*nt)
    return extractC_wt2(kk,ff,abs(vSpectr2)**2,u_mn,T_len,L_len[y])
