...
#        print(result[:,xWidth])
    ...
    return lon,lat,lev,result,corr_U,lon_rel,flow.m2deg_x(flow.um[ylev,xlev],ylev)

#===== main program =====
    
data_fname='npz/DiffuGrid350K_DJF_'+str(int(mask_sig))+'_'+str(yr_st)+'_'+str(yr_end)+'.npz'

lon,lat,lev,result,corr_U,lon_rel,u_lon=get_spectr(1980)
u_lon=u_lon*86400.
...
plt.savefig('/home/cjliu/Documents/RESEARCH/2017/DIFFUSIVITY/FIGURES/11_2017/auto_test'+str(xlev)+'_'+str(ylev)+'.pdf',format='pdf')
plt.show()
