...
plt_fld=Spectr_yz[100,:,:]/(nx*nt)**2
cs=axes[0,0].contourf(grid_x,grid_y,plt_fld,cmap=cm.RdBu_r,levels=np.arange(-4,4.1,0.5))
...
axes[0,0].set_title("UV on 300hPa")
#plt.colorbar(cs)
...
cc=np.arange(-50,50,dc)*1.0
grid_x,grid_y=np.meshgrid(cc,lat)
plt_fld=Spectr_yz2[100,:,:]/(nx*nt)**2
cs2=axes[0,1].contourf(grid_x,grid_y,plt_fld,cmap=cm.Oranges,levels=np.arange(0,40.1,2))
...
axes[0,1].set_title("VV on 300hPa")
#plt.savefig('Fig1_isen.pdf',format='pdf')

#=== plot variance verification ===
plt_rh_uv=2*np.sum(Spectr_yz[100,:,:],1)/(nx*nt)**2
plt_rh_vv=2*np.sum(Spectr_yz2[100,:,:],1)/(nx*nt)**2
plt_uv=variance_y2/37./(nx*nt)
plt_vv=variance_y/37./(nx*nt)
axes[1,0].plot(plt_uv,lat,linestyle='-',linewidth=2.5)
axes[1,0].plot(plt_rh_uv,lat,linestyle='--',color='red',linewidth=2.5)
axes[1,0].set_ylim(-85,85)

axes[1,1].plot(plt_vv,lat,linestyle='-',linewidth=2.5)
axes[1,1].plot(plt_rh_vv,lat,linestyle='--',color='red',linewidth=2.5)
axes[1,1].set_ylim(-85,85)

plt.savefig('/home/cliu/Documents/RESEARCH/2017/DIFFUSIVITY/FIGURES/11_2017/rh91_300mb.pdf',format='pdf')
plt.show()
